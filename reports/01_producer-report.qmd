...
# Filter data by depth parameters
# NULL values mean no filter on that boundary
if (!is.null(params$depth_min) || !is.null(params$depth_max)) {
  # Keep rows whose depth range overlaps the requested range
  if (!is.null(params$depth_min)) {
    data <- data |>
      dplyr::filter(end_depth_inches > params$depth_min)
  }
  if (!is.null(params$depth_max)) {
    data <- data |>
      dplyr::filter(start_depth_inches < params$depth_max)
  }

  if (nrow(data) == 0) {
    depth_desc <- if (!is.null(params$depth_min) && !is.null(params$depth_max)) {
      glue::glue("{params$depth_min}-{params$depth_max} inches")
    } else if (!is.null(params$depth_min)) {
      glue::glue(">{params$depth_min} inches")
    } else {
      glue::glue("<{params$depth_max} inches")
    }
    cli::cli_abort(c(
      "No samples found for depth range {.val {depth_desc}}.",
...
```{r depth-description}
#| output: asis
# Always use the actual data range, not the filter parameters
